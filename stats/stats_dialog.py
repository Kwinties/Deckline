...
def _build_day_window(entries: List[dict], days: int) -> List[dict]:
    want_days = _last_n_days(days)
...
def _chart_range_button_css() -> str:
    return (
        "QPushButton {"
        "border-radius: 10px;"
        "font-weight: 800;"
...
class DecklineStatsDialog(QDialog):
    def __init__(self, parent=None) -> None:
        ...
        status_badge = QLabel("PREMIUM" if self._is_premium else "FREE")
        ...
        status_badge.setStyleSheet(
            ...
            + (
                ...
                if self._is_premium
                else "background: rgba(148,163,184,0.18); color: rgba(226,232,240,0.98); border: 1px solid rgba(148,163,184,0.35);"
            )
        )
        top.addWidget(status_badge, 0)
        ...
        if self.chart is not None:
            ...
            chart_layout.setContentsMargins(0, 6, 0, 8)
            ...
            chart_tab_widget = self._build_blurred_locked_screen_widget()
    ...
    def _build_paywall(self) -> QWidget:
        ...
        # Title
        h = QLabel("Unlock Charts with Deckline Premium")
        hf = h.font()
        hf.setPointSize(15)
        hf.setBold(True)
        h.setFont(hf)
    ...
    def _build_blurred_locked_screen_widget(self) -> QWidget:
        ...
        title = QLabel("Unlock Charts with Deckline Premium")
        tf = title.font()
        tf.setPointSize(15)
        tf.setBold(True)
        title.setFont(tf)
    ...
    def _on_chart_range_changed(self, days: int) -> None:
        self._chart_days = 30 if int(days) == 30 else 7
        self._sync_chart_range_buttons()
        self._render()

    def _sync_chart_range_buttons(self) -> None:
        if not hasattr(self, "btn7d") or not hasattr(self, "btn30d"):
            return
    ...
    def _render(self) -> None:
        ...
        if did == -1:
            ...
            points = _build_day_window(entries, self._chart_days)
